...
namespace WebApp.Command.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> CreateFile(int type)
        {
            ...
            switch (fileType)
            {
                case EFileType.Excel:
                    ...
                    fileCreateInvoker.SetCommand(new CreateExcelActionCommand<Product>(excelFile));

                    break;

                case EFileType.Pdf:
                    PdfFile<Product> pdfFile = new(products, HttpContext);
                    fileCreateInvoker.SetCommand(new CreatePdfActionCommand<Product>(pdfFile));
                    break;
        ...
        public async Task<IActionResult> CreateFiles()
        {
            ...
            fileCreateInvoker.AddCommand(new CreateExcelActionCommand<Product>(excelFile));
            fileCreateInvoker.AddCommand(new CreatePdfActionCommand<Product>(pdfFile));
